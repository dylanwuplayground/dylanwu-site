...
function DirectionalRocket({
  ...
  // Animated trail: very short exhaust, builds up extremely slowly
  const trailLength = useTransform(trailProgress, (v) => `${Math.min(v * 0.08, 6)}%`);
  const trailLengthUp = useTransform(trailProgress, (v) => `${Math.min((100 - v) * 0.08, 6)}%`);
  ...
  return (
    <>
      ...
      <motion.div
        ...
        style={{
          left: "140px",
          top: useTransform(trailProgress, (v) => `${Math.max(0, v - Math.min(v * 0.08, 6))}%`),
          height: trailLength,
          translateX: "-0.5px",
